...
export const precompilePalettes = async (
  ...
  const getPalette = (id: string, fallbackId: string) => {
    if (id === "dmg") {
      return DMG_PALETTE;
    }
  ...
  const getSpritePalette = (id: string, fallbackId: string) => {
...
VM_ACTOR_SET_SPRITESHEET_BY_REF .ARG2, .ARG1`,
      ...
      const bankEntityEvents =
        (entityType: "actor", entityScriptField = "script") =>
        (entity: Actor, entityIndex: number) => {
          if (
